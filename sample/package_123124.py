...
class Nanonis3ds:
    ...
    def __init__(self, filename):
        ...
        self.data = {}
        raw_data = file[idx + 2:]
        bpp = self.header["points"] * \
            len(channels) + self.header["n_parameters"]
            ...
            for j in range(self.header["y_pixels"]):
                for k in range(len(channels)):
                    ...
                    if numerical_data[start_index:end_index].size != 0:
                        ...
                        predata[i][j][channels[k]] = np.zeros(
...
class Grid:
    ...
    def plot(self, sweep_index=0, channel="Input 2 (V)"):
        ...
        if self.fft:
            ...
            self.linecut_ax = self.fig.add_subplot(
                223)  # Axes for linecut through grid
            ...
            self.linecut_ax = self.fig.add_subplot(
                122)  # Axes for linecut through grid
            self.linecut_ax.set_aspect("auto")

        # Plot grid
        self.im = self.plot_ax.imshow(
            np.flipud(self.data[channel][:, :, sweep_index]),
            extent=(0, self.header["x_size (nm)"],
                    0, self.header["y_size (nm)"]),
            cmap="Blues_r",
        )  # Check to make sure x_size and y_size aren't mixed up
        if self.fft:
            fft_array = np.absolute(np.fft.fft2(
            ...
            self.fft_colorbar = self.fig.colorbar(
                self.fft_plot, ax=self.fft_ax)
        ...
        if self.fft:
            self.fft_linecut_line = matplotlib.lines.Line2D(
                [0, 0], [0, 0], color="r")
        ...
        def update_linecut():
            ...
            p0 = xydata[0] * self.header["x_pixels"] / \
                self.header["x_size (nm)"]
            p1 = xydata[1] * self.header["y_pixels"] / \
                self.header["y_size (nm)"]
            ...
            length = np.hypot(xydata[1, 0] - xydata[0, 0],
                              xydata[0, 1] - xydata[1, 1])
    ...
    def show_spectra(self, channel=None, ax=None):
        ...
        x_pixel = int(
            np.floor(
                self.click[0] * self.header["x_pixels"] /
                self.header["x_size (nm)"]
            )
        )
        y_pixel = int(
            np.floor(
                self.click[1] * self.header["y_pixels"] /
                self.header["y_size (nm)"]
            )
        )
        if ax is None:
        ...
        # TO DO: test this
        x = (x_pixel + 0.5) * \
            self.header["x_size (nm)"] / self.header["x_pixels"]
        y = (y_pixel + 0.5) * \
            self.header["y_size (nm)"] / self.header["x_pixels"]
            ...
            for j in range(self.data[channel].shape[1]):
                ...
                try:
                    ...
                    if peak_choosing_method.lower() in ["track", "tracking"]:
                        peak_ind = peaks[np.argmin(
                            np.abs(peaks - last_peak_ind))]
...
class Linecut(interactive_colorplot.Colorplot):
    ...
    def point_transform(self, x, y):
        ...
        transformed_x = transformed[0] + \
            self.sxm_data.header["x_range (nm)"] * 0.5
        transformed_y = transformed[1] + \
            self.sxm_data.header["y_range (nm)"] * 0.5
