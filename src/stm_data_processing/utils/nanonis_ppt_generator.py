...
for _, Folderpath in enumerate(DataFolderpath):
    ...
    if SxmFiles:
        ...
        for i, topo_path in enumerate(SxmFiles):
            ...
            time_str = str(datetime.fromtimestamp(get_creation_time(topo_path))).split(
                "."
            )[0]
    ...
    if DatFiles:
        for i, sts_path in enumerate(DatFiles):
            ...
            # metadata
            raw_sts = nap.read.Spec(str(sts_path))
            try:
                current_A = raw_sts.signals["Current [AVG] (A)"][0]
            except Exception:
                current_A = raw_sts.signals["Current (A)"][0]
            setpointI_sts = current_A * 1e12
            setpointV_sts = (
                raw_sts.signals["Bias calc (V)"][0] * 1000 / get_divider(sts_path)
            )
            time_sts = str(datetime.fromtimestamp(get_creation_time(sts_path))).split(
                "."
            )[0]
            name_sts = "/".join(Path(sts_path).parts[-3:])
    ...
    if LinecutFiles:
        for i, lc_path in enumerate(LinecutFiles):
            ...
            setpointV_lc = (
                raw_lc.signals["sweep_signal"][0] * 1000 / get_divider(lc_path)
            )
    ...
    if MapFiles:
        for _, mappath in enumerate(MapFiles):
            ...
            time_map = str(datetime.fromtimestamp(get_creation_time(mappath))).split(
                "."
            )[0]
            ...
            for page_start in range(0, n_bias, 10):
                slide = prs.slides.add_slide(prs.slide_layouts[6])
                add_section_header(slide, name_folder)
                txt = slide.shapes.add_textbox(Cm(0.5), Cm(2.2), Inches(4), Inches(1))
                ...
                for idx_offset in range(10):
                    ...
                    if img_path.exists():
                        pic = slide.shapes.add_picture(str(img_path), Cm(xi), Cm(yi))
                        pic.rotation = angle_ppt
            ...
            if QPI_switch:
                ...
                ani = FuncAnimation(fig, anim_update_qpi, frames=n_bias, interval=100)
                ani.save(str(Storagepath / "animation_QPI.gif"), writer="pillow", fps=4)
                plt.close(fig)
                ...
                for page_start in range(0, n_bias, 10):
                    ...
                    if page_start == 0:
                        pic = slide.shapes.add_picture(
                            str(Storagepath / "animation_QPI.gif"), Cm(13.6), Cm(0.8)
                        )
                        pic.rotation = angle_ppt
                    for idx_offset in range(10):
                        bias_idx = page_start + idx_offset
            ...
            if mapI_switch:
                ...
                ani = FuncAnimation(fig, anim_update_cur, frames=n_bias, interval=100)
                ani.save(
                ...
                for page_start in range(0, n_bias, 10):
                    ...
                    slide.shapes.add_textbox(
                        Cm(0.5), Cm(2.2), Inches(4), Inches(1)
                    ).text = f"Current in Map  part #{part}/{total_parts}\n{name_map}"
                    if page_start == 0:
                        pic = slide.shapes.add_picture(
                            str(Storagepath / "animation_mapI.gif"), Cm(13.6), Cm(0.8)
                        )
                        pic.rotation = angle_ppt
                    for idx_offset in range(10):
                        bias_idx = page_start + idx_offset
