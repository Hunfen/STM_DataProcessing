...
> 注：上表"实测"列为**设计探针 PROBE** 的数值，仅用于标定阈值；交付实现（回归脚本 v5 打印）的对应实测值为：`M5` FPR = **0.0101（S1）/ 0.0111（S2）/ 0.0267（S4）**，低 SNR 场景（S3）**0.0435**；`M6` = **0.0197 px**（S1，SNR ≥ 50）；`M7` = **0.0535 px**（S1，SNR 15–50，逐类定位 rms₂D）；`M7b` = **0.0514 px**（S3，SNR 15–50）；`M8` 类（SNR < 15）= 0.1421 px（S1）；`M10` = **0.844（SNR ≥ 50）/ 0.834（15–50）**（均为 S1）；`M11` ≈ **0.01446 px**（S1，SNR ≥ 50 的 median σ_q）；`M12` = 模型预测 rms₂D **0.00907 / 0.00744 / 0.04587 / 0.01514 px**（S1/S2/S3/S4，均小于同场景逐点 rms₂D；注意此列与 `M7`/`M7b` 的逐类定位 rms₂D 不是同一个量）；`M18` ≈ **17 s**（本机多轮 16.4–18.3 s，25/25 PASS，预算 180 s）。阈值列不受此影响、一律未改。
