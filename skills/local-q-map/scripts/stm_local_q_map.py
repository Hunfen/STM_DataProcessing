...
def parse_args(argv=None):
    ...
    parser.add_argument("--basis-from", default=None,
    ...
    parser.add_argument("--lambda-nm", type=float, default=lq.DEFAULT_LAMBDA_NM,
    ...
    parser.add_argument("--amplitude-fraction", type=float,
                        ...
                        help="mask threshold as a fraction of the median amplitude "
                             "(default 0.10)")
    parser.add_argument("--basis-orientation-from", choices=("canvas", "report"),
                        default="canvas",
    ...
    parser.add_argument("--stm-lib", default=STM_LIB_DEFAULT,
                        help="STM_DataProcessing src directory (bragg_peak basis "
                             "conventions)")
...
def field_of_view(args, basis, n_px):
    ...
    if report_scale:
        ...
        return value, (f"correction report corrected_nm_per_px {per_px:.8f} nm/px "
...
def main(argv=None):
    ...
    if (args.basis_from is None) == (args.basis_px is None):
        raise SystemExit("exactly one basis source is required: --basis-from "
    ...
    basis = (lq.basis_from_report(args.basis_from) if args.basis_from
    ...
    orientation = {"source": "command line --basis-px (explicit basis)",
                   "applied": False, "delta_deg": 0.0, "canvas": None}
    if resolved["kind"] in ("correction_report_affine",
                            "correction_report_lawler_fujita"):
        if args.basis_orientation_from == "report":
            orientation["source"] = ("report orientation (forced by "
                                     "--basis-orientation-from report)")
        else:
            canvas_orientation = lq.canvas_ring_orientation(
                topo, size_nm, float(np.hypot(*resolved["b1_px"])),
                float(resolved["b1_angle_deg"]))
            orientation["canvas"] = canvas_orientation
            if canvas_orientation["status"] != "measured":
                orientation["source"] = (f"report orientation (the canvas could not "
                                         f"be measured: {canvas_orientation['status']})")
            elif abs(float(canvas_orientation["delta_deg"])) \
                    > lq.BASIS_RING_ORIENTATION_TOLERANCE_DEG:
                basis = lq.rebase_orientation(
                    basis, canvas_orientation["delta_deg"],
    ...
    nm_per_px_mismatch = bool(
        abs(nm_per_px_ratio - 1.0) > lq.NM_PER_PX_REL_TOLERANCE)
    ...
    canvas_check = lq.basis_canvas_check(
        topo, lq.ring_1x1_members_rad_px(b1_rad_px, b2_rad_px),
        float(np.hypot(*resolved["b1_px"])), n, width_px)
    ...
    emit(f"# basis b2 = ({resolved['b2_nm_inv'][0]:.6f}, {resolved['b2_nm_inv'][1]:.6f}) "
    ...
    emit(f"# basis nm/px used for the rad/nm -> rad/px conversion: "
    ...
    if orientation.get("canvas") is not None:
        ...
        if orientation["applied"]:
            emit(f"# basis orientation: the report orientation is replaced by the "
    ...
    if canvas_check["mismatch"] and args.strict:
        emit("# ERROR: --strict (the default): the basis-vs-canvas check flagged a "
    ...
    if nm_per_px_mismatch:
        emit(f"# WARNING: nm/px mismatch: the canvas gives L / N = {nm_per_px:.8f} "
    ...
    if resolved.get("report_lockin_lambda_nm") is not None:
        emit(f"# basis-source report lockin.lambda_nm (reference only, NOT the window "
             f"of this skill): {resolved['report_lockin_lambda_nm']:.4f} nm")
    emit(f"# window lambda = {args.lambda_nm:.4f} nm = {width_px:.4f} px, "
         f"amplitude fraction = {args.amplitude_fraction:.4f}")
    emit(f"# boundary: the periodic FFT pollutes the demodulated maps within about "
         f"0.65 * lambda = {border_band_px:.4f} px of the border (reported, not cut)")
    for record in cross_talk:
        if record["cross_talk"]:
            emit(f"# WARNING: q{record['q_i']} and q{record['q_j']} are "
    ...
    if boundary_warning:
        emit(f"# WARNING: lambda = {args.lambda_nm:.4f} nm exceeds L / 4 = "
    ...
    for index, ((h, k), vector) in enumerate(zip(specs, vectors, strict=True)):
        ...
        if valid_fraction <= 0.0:
            emit(f"# WARNING: {label} has no valid pixel (amplitude threshold "
        ...
        identity_error_deg = float(np.degrees(lq.wrap_pm_pi(
            np.angle(np.sum(field)) - np.angle(target))))
        ...
        lq.save_npy(artifacts["mask_npy"],
        ...
        if not args.no_figures:
            ...
            lq.save_map(artifacts["theta_png"], np.where(valid, wrapped_deg, np.nan),
                        cmap="twilight", vmin=-180.0, vmax=180.0)
            lq.save_map(artifacts["mask_png"],
                        np.asarray(valid, dtype=np.float64), cmap="gray",
                        vmin=0.0, vmax=1.0)
        written.extend(str(path) for path in artifacts.values())

        emit(f"# {label}: (h, k) = {lq.format_hk(h, k)} -> q_px "
             f"({q_px[0]:.4f}, {q_px[1]:.4f}), |q| = {float(np.hypot(*q_nm_inv)):.6f} "
             f"nm^-1, amplitude threshold {threshold:.6g}")
        emit(f"# {label}: amplitude median {amplitude_median:.6g}, FWHM "
        ...
        emit(f"# {label}: mask coverage {100.0 * valid_fraction:.3f} % "
        ...
        entries.append({
            ...
            "amplitude": {
                ...
                "min_valid": (float(np.min(amp_valid)) if amp_valid.size else float("nan")),
            },
            "theta": {
    ...
    settings = {
        ...
        "lambda_nm": float(args.lambda_nm),
        "lambda_px": float(width_px),
        "lambda_note": ("the window of this skill is a local Fourier window; it is NOT "
        ...
        "warnings": {
            ...
            "nm_per_px_mismatch_rule": ("|basis_nm_per_px / (L / N) - 1| > "
                                        f"{lq.NM_PER_PX_REL_TOLERANCE:g}"),
    ...
    emit("")
    emit(f"# written: {', '.join(written)}, {report_path}, {outdir / 'local_q_map.log'}")
    (outdir / "local_q_map.log").write_text("\n".join(LOG) + "\n")
    return 0
