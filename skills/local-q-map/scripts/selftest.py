...
def affine_report(
    basis_rad_px, n_px, size_nm, measured=True, ideal=False, orientation_offset_deg=0.0
):
    ...
    orientation = float(np.degrees(np.arctan2(b1[1], b1[0]))) + float(
...
def affine_r3_report(basis_rad_px, n_px, size_nm, labelled=True):
    ...
    return {
        ...
        "a_nm": 0.246,
        "a_ref_nm": float(np.sqrt(3.0) * a_1x1),
        "orientation_deg": orientation,
        "b1_measured_nm_inv_after": radius_r3,
        "b1_ideal_nm_inv": radius_r3,
        "implied_lattice_after": {
            "a_ref_nm": float(np.sqrt(3.0) * a_1x1),
            "a_1x1_nm": a_1x1,
        },
        "rings_after": [
            {
                "radius_px": float(ring_radius_px(n_px)),
                "radius_nm_inv": radius_1x1,
                "n_members": 12,
                "total_amplitude": 1.0,
            },
            {
                "radius_px": float(ring_radius_px(n_px)) / np.sqrt(3.0),
                "radius_nm_inv": radius_r3,
                "n_members": 6,
                "total_amplitude": 0.3,
            },
        ],
...
def check_single_plane_wave(basis, n_px, nm_per_px, columns, rows):
    ...
    check(
        "single plane wave: amplitude is A/2",
    ...
    check(
        "single plane wave: theta equals +phi0 with the demodulated sign",
    ...
    check(
        "single plane wave: theta is wrapped inside (-pi, pi]",
...
def check_multi_peak(basis, n_px, nm_per_px, columns, rows, lambda_px_value):
    section(
        "2. the 12 q of the synthetic (ring_1x1 + ring_r3): own A and phi, cross-talk"
    )
    table = peak_table()
    ...
    for peak in table:
        ...
        alone_image = plane_wave(
            columns, rows, q_line, peak["amplitude"], peak["phase"]
        )
        for sign in (1.0, -1.0):
            q = sign * q_line
            field = lq.demodulate(image, q, LAMBDA_NM, nm_per_px)
            ...
            dc_error = abs(complex(np.mean(field)) - complex(np.mean(alone))) / abs(
                complex(np.mean(alone))
            )
            ripple = float(np.max(np.abs(field - alone))) / float(
                np.median(np.abs(alone))
            )
    ...
    check(
        "every one of the 12 q recovers its own modulation phase (Friedel signed)",
    ...
    check(
        "cross-talk between the six modulations stays at the sampling floor",
    ...
    check(
        "the pointwise inter-component leakage stays at the 24 / N floor",
...
def check_basis_px_equivalence(env, workdir, csv_path, report_path, size_nm):
    ...
    escape = (
        f"{basis['b1_px'][0]:.17g},{basis['b1_px'][1]:.17g};"
        f"{basis['b2_px'][0]:.17g},{basis['b2_px'][1]:.17g}"
    )
    completed_px = run_script(
        "stm_local_q_map.py",
        [
            str(csv_path),
            "-o",
    ...
    check(
        "the same vector given as (h, k) or as --basis-px gives the same map",
...
def check_basis_sources(env, workdir, csv_path, size_nm, basis_rad_px, n_px):
    section(
        "4. basis parsing: lawler-fujita style, affine style, affine anchored on "
    ...
    if completed.returncode == 0:
        ...
        difference = float(np.max(np.abs(found - expected))) / float(
            np.hypot(*expected)
        )
        check(
            "lawler-fujita style report: b1 = q_a, b2 = q_a + q_b",
        ...
        check(
            "the log prints the basis report's lockin.lambda_nm for reference only",
            "lockin.lambda_nm (reference only" in log and "30.0000 nm" in log,
            "log line: "
            + next(
                (line for line in log.splitlines() if "lockin.lambda_nm" in line),
                "MISSING",
            ),
        ...
        check(
            "lawler-fujita style report is accepted",
            False,
            f"exit code {completed.returncode}: {completed.stderr.strip()[-300:]}",
        )
    ...
    if completed.returncode == 0:
        ...
        check(
            "affine style report: b1 = R (cos t, sin t), b2 = b1 rotated +60 deg",
            payload["kind"] == "correction_report_affine"
            and difference <= 1e-12
        ...
        check(
            "affine style report is accepted",
            False,
            f"exit code {completed.returncode}: {completed.stderr.strip()[-300:]}",
        )
    ...
    fallbacks = []
    fallback_ok = True
    for label, measured, ideal in (
        ("b1_ideal_nm_inv", False, True),
        ("implied_lattice_after.a_1x1_nm", False, False),
    ):
        path = workdir / f"report_fallback_{label.split('.')[0]}.json"
        write_json(
            path,
            affine_report(basis_rad_px, n_px, size_nm, measured=measured, ideal=ideal),
        )
    ...
    check(
        "the affine radius falls back to b1_ideal_nm_inv and to a_1x1_nm",
        fallback_ok,
        "; ".join(fallbacks),
        "relative 1e-12",
    )
    ...
    for label, labelled in (("anchor_ring=r3", True), ("no anchor_ring", False)):
        ...
        write_json(
            path, affine_r3_report(basis_rad_px, n_px, size_nm, labelled=labelled)
        )
        ...
        if completed.returncode != 0:
            check(
                f"the ring_r3-anchored affine report is accepted ({label})",
                False,
                f"exit code {completed.returncode}: {completed.stderr.strip()[-300:]}",
            )
            continue
        ...
        check(
            f"an affine report anchored on ring_r3 resolves the ring_1x1 basis "
            f"({label})",
            relative <= 1e-12
        ...
        if labelled:
            ...
            check(
                "the log and the report notes name the decoded anchored ring",
                ...
                + next(
                    (
                        ...
                        if "anchored on ring_r3" in line
                    ),
                    "MISSING",
                )
    ...
    check(
        "the rings_after cross-check warns only when the basis sits on ring_r3",
    ...
    write_json(
        rotated_path,
        affine_report(basis_rad_px, n_px, size_nm, orientation_offset_deg=rotation_deg),
    )
    ...
    if completed_rotated.returncode != 0:
        ...
        if completed_report_frame.returncode != 0 or completed_strict.returncode == 0:
            check(
                "the basis-vs-canvas check flags the report frame and the default "
    ...
    check(
        "a report without a recognised basis exits non-zero with a clear message",
...
def check_gauge_law(n_px, nm_per_px, columns, rows):
    ...
    shifted = synthetic_image(
        columns, rows, basis, [dict(peak, phase=peak["phase"] + phi0) for peak in table]
    )
    worst_phase = 0.0
    for peak in table:
        q = lq.q_vector(peak["h"], peak["k"], basis[0], basis[1])
        ...
        worst_phase = max(
            worst_phase, float(np.max(np.abs(np.degrees(lq.wrap_pm_pi(b - a - phi0)))))
        )
    check(
        "a constant modulation phase phi0 shifts the theta map by exactly phi0",
        ...
        worst_amplitude = max(
            ...
            float(
                np.max(
                    np.abs(
                        np.abs(after)
                        - np.roll(np.abs(before), shift=shift, axis=(0, 1))
                    )
                )
            )
            / scale,
    ...
    check(
        "shifting the image by delta translates the maps by delta",
    ...
    check(
        "shifting the image by delta translates the amplitude map by delta",
...
def check_identities(basis, n_px, nm_per_px, columns, rows):
    ...
    for peak in table:
        ...
        worst_friedel = max(
            worst_friedel,
            float(np.max(np.abs(np.degrees(lq.wrap_pm_pi(theta + theta_minus))))),
        )
    check(
        "theta_q + theta_-q = 0 mod 2 pi for a real image",
    ...
    for peak in table:
        for sign in (1.0, -1.0):
            ...
            worst_mean = max(
                worst_mean, abs(float(np.degrees(lq.wrap_pm_pi(left - right))))
            )
    check(
        "the window cancels from the total phase (the strongest exact identity)",
        worst_mean <= MEAN_IDENTITY_TOL_DEG,
        f"worst |arg sum psi - arg sum T exp(-i q.r)| = {worst_mean:.3e} deg over "
        "all 12 q",
        f"{MEAN_IDENTITY_TOL_DEG:g} deg",
    )
...
def check_nan_handling(env, workdir, basis, n_px, nm_per_px, columns, rows, size_nm):
    ...
    residual = float(
        np.linalg.norm((spectrum_zero - spectrum_plane - predicted).ravel())
    )
    reference = float(np.linalg.norm(spectrum_zero.ravel()))
    relative = residual / reference
    check(
        "fill-0 and fill-plane differ in the spectrum only through the gap",
    ...
    row_gap = np.maximum(
        np.maximum(
            np.arange(n_px)[:, None] - patch[1], patch[0] - np.arange(n_px)[:, None]
        ),
        0.0,
    )
    col_gap = np.maximum(
        np.maximum(
            np.arange(n_px)[None, :] - patch[3], patch[2] - np.arange(n_px)[None, :]
        ),
        0.0,
    )
    ...
    near_relative = (
        float(np.max(np.abs(field_zero[~halo] - field_plane[~halo]))) / scale
    )
    check(
        "the fill value perturbs the demodulated maps by less than the gap itself",
    ...
    completed = run_script(
        ...
        [
            ...
            str(outdir),
            "--basis-from",
            str(workdir / "report_affine.json"),
            "--q",
            "1,0",
            "--no-figures",
            "-L",
    ...
    if completed.returncode != 0:
        check(
            "the NaN pipeline run completes",
            False,
            f"exit code {completed.returncode}: {completed.stderr.strip()[-300:]}",
        )
        return
...
def check_determinism(env, workdir, csv_path, report_path, size_nm):
    ...
    if completed_a.returncode != 0 or completed_b.returncode != 0:
        check(
            "both determinism runs complete",
            False,
...
# --------------------------------------------------------------------------- #
def check_size_nm_from_log(
    env, workdir, csv_path, basis_px_spec, n_px, lf_path, size_nm
):
    section(
        "10. field of view: --size-nm-from-log branches and the correction-report "
        "branch"
    )
    ...
    if completed.returncode == 0:
        payload = read_json(outdir / "local_q_map_report.json")
        check(
            "the corrected-canvas line of the log wins over the input-canvas line",
        ...
        check(
            "the corrected-canvas log branch completes",
            False,
            f"exit code {completed.returncode}: {completed.stderr.strip()[-300:]}",
        )
    ...
    if completed.returncode == 0:
        ...
        check(
            "a single-line log falls back to its last field of view",
        ...
        check(
            "the single-line log branch completes",
            False,
            f"exit code {completed.returncode}: {completed.stderr.strip()[-300:]}",
        )
    ...
    message = completed.stderr + completed.stdout
    check(
        "a log without any field of view line is an explicit error",
    ...
    if completed.returncode == 0:
        ...
        check(
            "the report branch times corrected_nm_per_px by the canvas",
        ...
        check(
            "the report branch keeps the window inside the canvas",
        ...
        check(
            "the report field-of-view branch completes",
            False,
            f"exit code {completed.returncode}: {completed.stderr.strip()[-300:]}",
        )
...
def check_end_to_end(env, workdir, csv_path, lf_path, affine_path, n_px, size_nm):
    ...
    if completed.returncode != 0:
        check(
            "the single-q pipeline run completes",
            False,
            f"exit code {completed.returncode}: {completed.stderr.strip()[-300:]}",
        )
        return None, None
    ...
    if completed_multi.returncode != 0:
        check(
            "the multi-q pipeline run completes",
            False,
    ...
    absent = sorted(
        {name for entry in entries for name in required if name not in entry}
    )
    stats_absent = sorted(
        {
            name
            for entry in entries
            for name in ("median", "fwhm")
            if name not in entry["amplitude"]
        }
    )
    theta_absent = sorted(
        {
            ...
            for name in (
                "circular_mean_deg",
                "circular_mean_all_pixels_deg",
                "mean_identity_error_deg",
                "circular_median_deg",
                "resultant_R",
            )
            if name not in entry["theta"]
        }
    )
    check(
        "the multi-q report carries every per-q contract field",
    ...
    check(
        "the q labels and the (h, k) bookkeeping follow the command line",
        labels == ["q0", "q1", "q2"]
        and coordinates
        == [(1.0, 0.0), (0.0, 1.0), (round(1.0 / 3.0, 12), round(1.0 / 3.0, 12))],
        f"labels {labels}, (h, k) {coordinates}",
        "q0, q1, q2 and (1,0), (0,1), (1/3,1/3)",
    )
    ...
    check(
        "the report carries the basis-source and warning fields",
        not source_absent
    ...
    needles = (
        ...
        "# q0:",
        "# written:",
        "boundary: the periodic FFT",
    )
    absent_needles = [needle for needle in needles if needle not in log]
    check(
        "the log carries the contract lines",
        not absent_needles,
        f"{len(needles) - len(absent_needles)}/{len(needles)} lines present; "
        f"missing {absent_needles or 'none'}",
        "every line present",
    )

    check(
        "no k-space high-symmetry name appears in the products",
        all(
            token not in log.lower()
            for token in ("bragg point", "high-symmetry", "high symmetry", " k-point")
        ),
    ...
    bimodal = amplitude_map * np.cos(q_line[0] * columns + q_line[1] * rows + phase_map)
    ...
    valid, _ = lq.valid_mask(
        amplitude, ~np.isfinite(bimodal), lq.DEFAULT_AMPLITUDE_FRACTION
    )
    theta = np.angle(field)
    ...
    if completed.returncode == 0:
        reported = float(
            read_json(bimodal_out / "local_q_map_report.json")["q"][0]["theta"][
                "circular_median_deg"
            ]
        )
        gap = abs(float(np.degrees(lq.wrap_pm_pi(weighted - unweighted))))
        residual = abs(
            float(np.degrees(lq.wrap_pm_pi(np.radians(reported) - weighted)))
        )
        check(
            "the reported circular median is the amplitude-weighted one",
        ...
        check(
            "the bimodal-amplitude run completes",
            False,
            f"exit code {completed.returncode}: {completed.stderr.strip()[-300:]}",
        )
    ...
    check(
        "a canvas that agrees with the basis source raises no nm/px mismatch",
        ...
        f"mismatch flag {matching['nm_per_px_mismatch']}",
        "ratio 1 and flag false",
    )
    check(
        "the end-to-end run passes the basis-vs-canvas check",
    ...
    if completed_mismatch.returncode == 0:
        ...
        check(
            "a field of view that disagrees with the basis source warns",
        ...
        check(
            "the mismatched field-of-view branch completes",
            False,
    ...
    if completed_cross.returncode == 0 and completed_boundary.returncode == 0:
        ...
        check(
            "a q pair closer than 2 / lambda raises the cross-talk warning",
        ...
        check(
            "lambda above L / 4 raises the boundary warning",
        ...
        check(
            "the two warning branches complete",
            False,
...
def main(argv=None):
    ...
    try:
        ...
        check_multi_peak(
            basis, n_px, nm_per_px, columns, rows, lq.lambda_px(LAMBDA_NM, nm_per_px)
        )
    ...
    for _index, item in enumerate(SECTIONS, start=1):
        ...
        print(
            f"  [{'PASS' if ok else 'FAIL'}] {item['title']}"
            + (f" -- failed: {failures}" if failures else "")
        )
    print(
        f"  {passed_sections}/{len(SECTIONS)} checks passed "
