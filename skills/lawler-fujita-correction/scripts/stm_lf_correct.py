...
def parse_args(argv=None):
    ...
    parser.add_argument(
        ...
        help="minimum valid-pixel fraction before the fit falls back (default 0.50)",
    )
    parser.add_argument(
        "--ring-tol",
        type=float,
    ...
    parser.add_argument(
        "--order", type=int, default=3, help="spline order of the warp (default 3)"
    )
    parser.add_argument(
        "--patch-half",
        type=int,
    ...
    *ideal* hexagon: the radius comes from ``--a`` and the orientation from the data
    ...
    def nearest(signed_target_deg):
        ...
        for member in members[1:]:
            delta = (
                (np.degrees(np.arctan2(member[1], member[0])) - angle_a + 180.0) % 360.0
            ) - 180.0
    ...
    if second is None:
        raise ValueError(
            f"no ring member {pair_angle:g} degrees counter-clockwise from "
    ...
    if third is None and float(pair_angle) == 120.0:
        raise ValueError(
            "no ring member 120 degrees clockwise from the strongest "
    ...
    for member, slot in zip(measured, slots, strict=True):
        delta = (
            (np.degrees(np.arctan2(member[1], member[0])) - angle_a - slot + 180.0)
            % 360.0
        ) - 180.0
...
def reference_vectors(orientation_deg, slots, size_nm, a_nm):
    ...
    return np.array(
        [
            [
                radius * np.cos(np.radians(orientation_deg + slot)),
...
def u_statistics(u_nm, valid):
    ...
    stats["norm"] = {
        "rms_nm": float(np.sqrt(np.mean(norm**2))) if norm.size else 0.0,
...
def write_artifacts(prefix, theta, amplitude, u_nm, valid):
    ...
    written = {}
    entries = [
        ("theta_a", theta[0], "phase"),
        ("theta_b", theta[1], "phase"),
        ("amplitude_a", amplitude[0], "amplitude"),
        ("amplitude_b", amplitude[1], "amplitude"),
        ("u_x", u_nm[0], "displacement"),
        ("u_y", u_nm[1], "displacement"),
...
def bundle_payload(
    report_path, csv_path, args, q_pair_px, report, n_out, u_path, u_stats
):
    ...
    return {
        ...
        "q_a_nm_inv": [
            float(value) for value in lf.wave_vectors_nm_inv(q_pair_px[0], args.size_nm)
        ],
        "q_b_nm_inv": [
            float(value) for value in lf.wave_vectors_nm_inv(q_pair_px[1], args.size_nm)
        ],
        ...
        "amplitude_threshold": (
            None
            if report["lockin"]["amplitude_threshold"] is None
...
def main(argv=None):
    ...
    gauge = (
        "theta_bar = 0: the mean local phase over the valid pixels is removed, "
    ...
    emit(
        f"# ideal 1x1 ring radius {radius_ideal:.4f} px = "
    ...
    if matched is None:
        fallback_reason = (
            f"no detected ring with at least six members matches the "
        ...
        emit("")
        emit(
            f"# anchor ring: the detected 1x1 ring (radius {float(matched['radius']):.4f} "
    ...
    if fallback_reason is None:
        try:
            directions, slots, orientation, deviation = pick_directions(
                matched, args.pair_angle
            )
            ...
            emit(
                f"# reference hexagon: ideal radius {radius_ideal:.4f} px from a = "
            ...
            emit(
                f"# lock-in directions: {n_directions} "
            ...
            for member, q_vector, name in zip(directions, q_pair, names, strict=True):
                emit(
                    f"# lock-in direction {name}: measured q_px = "
            ...
            offset_cycles = float(
                np.max(np.hypot(*(q_measured - q_pair).T)) / args.size_nm
            )
            band_margin = float(args.lambda_nm) * offset_cycles
            emit(
                f"# reference vs measured peak: offset {offset_cycles:.4f} cycles/nm, "
            ...
            if band_margin > 1.0:
                emit(
                    "# WARNING: the low-pass is too narrow for this data: the measured "
    ...
    if fallback_reason is None:
        u_nm = lf.displacement_from_phase(
            theta_maps[0], theta_maps[1], q_pair[0], q_pair[1], args.size_nm
        )
        u_stats = u_statistics(u_nm, valid)
        gradients = [
            lf.max_phase_step(theta_maps[index]) for index in range(theta_maps.shape[0])
        ]
        emit("")
        emit("== lock-in ==")
        emit(
            f"# low-pass exp(-lambda^2 k^2 / 2) with lambda = {args.lambda_nm:g} nm "
            f"(keeps distortion structure on real-space scales >= {args.lambda_nm:g} nm)"
        )
        emit(
            f"# amplitude threshold {args.amplitude_fraction:g} x median = {threshold:.4e} "
            f"-> valid pixels {100 * coverage:.2f} %"
        )
        emit(
            f"# largest per-pixel phase step {max(gradients):.4f} rad (the unwrap is "
        ...
        emit(f"# gauge: {gauge}")
        emit(
            f"# u RMS {u_stats['norm']['rms_nm']:.4f} nm (u_x {u_stats['u_x']['rms_nm']:.4f}, "
        ...
        if theta_maps.shape[0] == 3:
            ...
            margin = min(
                int(
                    np.ceil(LAMBDA_BORDER_MARGIN * args.lambda_nm / (args.size_nm / n))
                ),
                n // 4,
            )
            ...
            if int(np.count_nonzero(interior)) > 0:
                interior_rms = lf.wrapped_residual(np.degrees(theta_maps[:, interior]))[
                    0
                ]
            third = {
                "available": True,
                "wrapped_rms_deg": residual,
            ...
            third = {
                "available": False,
                "reason": (
                    "the two lock-in directions are 60 deg apart, so "
        ...
        emit(
            f"# warp: corrected(r) = T(r + u(r)), spline order {args.order}, "
        ...
        emit(
            "# WARNING: identity fallback: no correction is applied, the input is copied "
    ...
    before_quality = ring_quality(
        detection.peaks, args.size_nm, args.a, args.ring_cluster_tol, args.ring_tol
    )
    after_quality = ring_quality(
        after.peaks, size_out, args.a, args.ring_cluster_tol, args.ring_tol
    )
    if after_quality["ring_1x1"] is None:
        ...
        if ring_before is not None:
            emit(
                f"# residual self-check: 1x1 anisotropy before "
    ...
    if after_quality["r3_ratio_vs_sqrt3"] is not None:
        ...
        emit(
            f"# residual self-check: 1x1/r3 ring pair ratio {pair['ratio']:.6f} "
            f"({100 * pair['deviation']:+.4f} % from sqrt(3) = {np.sqrt(3.0):.6f})"
        )
    elif after_quality["ring_1x1"] is not None:
        emit(
            "# residual self-check: no r3 ring on the corrected image, the 1 : sqrt(3) "
    ...
    report = {
        ...
        "q_a_px": (
            [float(value) for value in q_pair[0]] if q_pair is not None else None
        ),
        "q_b_px": (
            [float(value) for value in q_pair[1]] if q_pair is not None else None
        ),
        "q_c_px": (
            [float(value) for value in q_pair[2]]
            if q_pair is not None and q_pair.shape[0] > 2
            else None
        ),
        "q_measured_px": (
            [[float(value) for value in row] for row in q_measured]
        ...
        "q_a_nm_inv": (
            [float(value) for value in lf.wave_vectors_nm_inv(q_pair[0], args.size_nm)]
            if q_pair is not None
            else None
        ),
        "q_b_nm_inv": (
            [float(value) for value in lf.wave_vectors_nm_inv(q_pair[1], args.size_nm)]
            if q_pair is not None
            else None
        ),
        "q_c_nm_inv": (
            [float(value) for value in lf.wave_vectors_nm_inv(q_pair[2], args.size_nm)]
            if q_pair is not None and q_pair.shape[0] > 2
            else None
        ),
        "q_matrix_px": (
            [[float(value) for value in row] for row in q_pair]
            if q_pair is not None
            else None
        ),
        "lockin": {
            "lambda_nm": float(args.lambda_nm),
            "reference_offset_cycles_per_nm": (
                None if q_pair is None else float(offset_cycles)
            ),
            "band_margin_lambda_times_offset": (
                None if q_pair is None else float(band_margin)
            ),
        ...
        "transfer_constraints": {
            "same_scan": (
                "the target must be the same scan and the same field of view: "
                "the bundle carries a dense displacement field of the "
                "reference grid"
            ),
            "grid_rescaling": (
                "a different pixel count is supported: u is physical, so "
                "it is interpolated onto the target pixel coordinates"
            ),
            "lattice_not_required": (
                "the target need not show a lattice at all -- "
    ...
    if args.save_transform:
        ...
        if u_nm is not None:
            ...
            payload = bundle_payload(
                report_path, csv_path, args, np.zeros((2, 2)), report, n_out, u_path, {}
            )
        bundle_path.write_text(json.dumps(payload, indent=2) + "\n")
        print(
            f"# bundle written: {bundle_path}"
            + (
                f" (+ {u_path})"
                if u_nm is not None
